...
class GeoNames(Geocoder): # pylint: disable=W0223
    ...
    def __init__(
        ...
        self.api_postalcode_reverse = (
            "%s://api.geonames.org/findNearbyPostalCodesJSON" % self.scheme
        )
    ...
    def reverse(
        ...
        for key in extra.keys():
            params[key] = extra[key]
        if self.country_bias:
            params['lang'] = self.country_bias
        url = "?".join((url, urlencode(params)))
        logger.debug("%s.reverse: %s", self.__class__.__name__, url)
        return self._call_geocoder(url, timeout=timeout)

    def reverse_postal_code(self,
                            query,
                            exactly_one=False,
                            timeout=None,
                            extra={},
                            url=""):
        return self.reverse(query=query,
                            exactly_one=exactly_one,
                            timeout=timeout,
                            extra=extra,
                            url=self.api_postalcode_reverse)
